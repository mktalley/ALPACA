...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Format zero_dte.log for human readability"
    )
    parser.add_argument(
        "--input",
        "-i",
        default="apps/logs/zero_dte.log",
        help="Path to zero_dte.log file (default apps/logs/zero_dte.log)",
    )
...
def main():
    ...
    price_re = re.compile(
        r"^(?P<ts>\d{4}-\d{2}-\d{2} \d{2}:\d{2}:\d{2}).*?DEBUG.*?\[(?:Thread-[^\]]+)\] Current prices \[(?P<syms>[^\]]+)\] → (?P<val>[0-9.]+) \(sum\)"
    )
    generic_re = re.compile(
        r"^(?P<ts>\d{4}-\d{2}-\d{2} \d{2}:\d{2}:\d{2}).*?(?P<lvl>INFO|WARNING|ERROR)\s+\[[^\]]+\] (?P<msg>.*)"
    )
