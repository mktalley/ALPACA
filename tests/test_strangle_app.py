...
def test_choose_otm_strangle(monkeypatch, fake_chain):
    ...
    monkeypatch.setattr(
        "apps.0dte_app.TradingClient.get_option_contracts",
        lambda self, req: fake_chain,
    )
    monkeypatch.setattr(
        "apps.0dte_app.get_underlying_price",
...
def test_submit_strangle(monkeypatch):
    ...
    monkeypatch.setattr(
        "apps.0dte_app.TradingClient.submit_order", fake_submit
    )
...
def test_monitor_and_exit_strangle(monkeypatch):
    ...
    monkeypatch.setattr(
        "apps.0dte_app.OptionHistoricalDataClient.get_option_latest_trade",
        fake_trade,
    )
    calls = []
